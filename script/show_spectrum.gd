...
var min_values = []
var max_values = []
@export var c1:Color=Color(1,1,0.8,0.7)
@export var c2:Color=Color(0.6,0.6,0,0.7)
...
func _draw():
	...
	for i in range(VU_COUNT):
		...
		draw_rect(
				...
				c1
				#Color.from_hsv(float(VU_COUNT * 0.3 + i * 0.7) / VU_COUNT, 0.3, 0.8)
		)
		draw_line(
				Vector2(w * i, HEIGHT - height),
				Vector2(w * i + w - 2, HEIGHT - height),
				c2,
				#Color.from_hsv(float(VU_COUNT * 0.3 + i * 0.7) / VU_COUNT, 0.4, 1.0),
				2.0,
				true
		)
		...
		#draw_rect(
				...
				#Color(1,1,1,1)* Color(1, 1, 1, 0.125)
		...
		#draw_line(
				...
				#Vector2(w * i + w - 2, HEIGHT + height),
				#
				#Color.from_hsv(float(VU_COUNT * 0.3 + i * 0.7) / VU_COUNT, 0.7, 1.0) * Color(1, 1, 1, 0.125),
				#2.0,
				#true
		#)
